# This script will execute a general bridged setup for a router with both 2.4 & 5.8
...
/interface bridge port add bridge=bridge-internal interface=wlan2

#### Setting up Identities
#Setting up wireless interface
/interface wireless set wlan1 mode=ap-bridge disabled=no ssid="$id 2.4" band=2ghz-b/g/n frequency-mode=manual-txpower country="south africa" wireless-protocol=802.11 security-profile=$id radio-name="$id 2.4" name="$id 2.4"
/interface wireless set wlan2 mode=ap-bridge disabled=no ssid="$id 5.8" band=5ghz-a/n frequency-mode=manual-txpower country="south africa" wireless-protocol=802.11 security-profile=$id radio-name="$id 5.8" name="$id 5.8"
...
/file remove [/file find where name=bridged_AC2.rsc]
